...
module.exports = function(jsonapi, data, opts) {
  ...
  function findIncluded(relationshipData, relationshipName, from) {
    return new Promise(function(resolve) {
      ...
      var includes = _filter(jsonapi.included, {
        id: relationshipData.id,
        type: relationshipData.type,
      });

      var path = [from.type, from.id, relationshipName, relationshipData.type, relationshipData.id];
      ...
      if (includes && includes.length > 0) {
        var included = includes[0];
        return Promise.all([extractAttributes(included), extractRelationships(included)]).then(
          function(results) {
    ...
    return Promise.all(
      Object.keys(from.relationships).map(function(key) {
        ...
        if (relationship.data === null) {
          ...
          return extractIncludes(relationship.data, key, from).then(function(include) {
            if (include) {
  ...
  function extractIncludes(relationshipData, relationshipName, from) {
    return findIncluded(relationshipData, relationshipName, from).then(function(included) {
  ...
  this.perform = function() {
    return Promise.all([extractAttributes(data), extractRelationships(data)]).then(function(
      results,
    ) {
      var attributes = results[0];
      var relationships = results[1];
